...
from helomi.cli.logs import LogBuffer, configure_ui_logger
from helomi.cli.progress import (
    HuggingFaceProgress,
    ProgressStatus,
    ProgressStore,
    _ProgressTqdm,
...
def test_log_buffer_is_bounded_thread_safe_sink() -> None:
    ...
    logger.info("ready")
    logger.debug("hidden")
    assert any("ready" in line and "Helomi" in line for line in buffer.drain())
...
def test_hugging_face_progress_injects_and_restores_adapter(
    ...
    with adapter:
        ...
        assert snapshot_module.hf_hub_download("install", tqdm_class=str) == "install"
        with pytest.raises(RuntimeError, match="already installed"):
            adapter.__enter__()
    assert calls == [
        ("snapshot", _ProgressTqdm),
        ("file", str),
        ("install", _ProgressTqdm),
    ]
