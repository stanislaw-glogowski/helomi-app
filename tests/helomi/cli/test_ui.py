...
from helomi.cli.events import UiEventBridge
from helomi.cli.logs import LogBuffer
from helomi.cli.progress import ProgressStore
from helomi.cli.ui import TerminalApp
from helomi.cli.ui.screens import ProfilePicker, StartupScreen
from helomi.cli.ui.state import (
    AssistantMessage,
    AssistantPhrase,
    ConversationState,
    PhraseState,
...
from helomi.cli.ui.widgets import (
...
from helomi.conversation.events import ConversationReady
...
def test_dashboard_renderables_expose_runtime_and_conversation_states() -> None:
    ...
    assert interrupted_panel.title.plain == "Test Agent"
...
def test_conversation_follows_latest_until_user_scrolls_history() -> None:
    ...
    async def scenario() -> None:
        ...
        async with app.run_test(size=(70, 12)) as pilot:
            ...
            view.assistant_name = "Test Agent"
            view.wakeword_label = "Hey Agent"
            await pilot.pause()
            empty = transcript.query_one("#conversation-empty", Static)
            assert "Waiting for “Hey Agent”…" in rendered(empty.render())
            ...
            assert active_panel.title.plain == "Test Agent"
...
def test_startup_screen_operations_are_marshaled_to_textual_task() -> None:
    async def scenario() -> None:
        ...
        try:
            ...
            assert isinstance(app.screen, StartupScreen)
            assert "AGENT  /  INITIALIZING" in rendered(
                app.screen.query_one("#startup-kicker", Static).render()
            )
...
def test_profile_and_startup_modal_cancel_paths() -> None:
    async def scenario() -> None:
        ...
        async with app.run_test(size=(90, 30)) as pilot:
            ...
            assert "Agent" in rendered(fallback.prompt)
...
def test_profile_picker_places_default_first() -> None:
    async def scenario() -> None:
        entries = (
            ...
            ProfileEntry("alexa", "Alexa", profile()),
