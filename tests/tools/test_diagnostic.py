...
from helomi.speech.audio import AudioFormat, AudioPlaybackOutcome
from tools.diagnostic.cli import (
    main,
    measure_full_duplex,
    record_and_play,
)
...
def test_main_uses_configured_or_explicit_driver(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tools.diagnostic.cli.LocalStore", lambda: store)
    monkeypatch.setattr(
        "tools.diagnostic.cli.get_audio_driver",
        lambda settings: selected.append(settings.driver) or driver,
    )
    monkeypatch.setattr(
        "tools.diagnostic.cli.record_and_play",
        lambda actual_driver, seconds: selected.extend((actual_driver, seconds)),
    )
    monkeypatch.setattr(
        "tools.diagnostic.cli.measure_full_duplex",
        lambda actual_driver, seconds: selected.extend(
            ("duplex", actual_driver, seconds)
        ),
    )

    main(["--type", "audio", "--seconds", "2"])
    main(["--type", "audio", "--seconds", "3", "--driver", "pyaudio"])
    main(["--type", "audio", "--seconds", "4", "--duplex"])
