...
from helomi.conversation.profile import ConversationProfile, ConversationPrompts
from tests.support import FakeLanguageModel
from tools.benchmark import cli as router_module
from tools.benchmark import conversation as cli_module
from tools.benchmark.conversation import override_model_id
from tools.benchmark.core import (
    BenchmarkCase,
    load_cases,
    run_benchmark,
    write_report,
...
""".strip(),
    ...
    monkeypatch.setattr(
        "tools.benchmark.core.get_language_model",
...
def test_conversation_benchmark_cli_uses_configured_adapter(
    ...
    assert output_directories[0].name.startswith("alexa-mlx-")
    assert output_directories[0].parents[1].name == "conversation"


def test_benchmark_router_dispatches_by_type(monkeypatch) -> None:
    calls: list[tuple[str, list[str]]] = []
    monkeypatch.setattr(
        router_module.conversation,
        "main",
        lambda argv: calls.append(("conversation", argv)) or 0,
    )
    monkeypatch.setattr(
        router_module,
        "voice_main",
        lambda argv: calls.append(("voice", argv)) or 0,
    )

    assert router_module.main(["--type", "conversation", "--profile", "alexa"]) == 0
    assert router_module.main(["--type", "voice", "list"]) == 0
    assert calls == [
        ("conversation", ["--profile", "alexa"]),
        ("voice", ["list"]),
    ]
