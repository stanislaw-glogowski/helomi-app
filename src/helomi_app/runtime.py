...
from helomi_core.audio import AudioDriver, get_audio_driver
from helomi_core.detection import DetectionWorker
from helomi_core.stt import STTWorker, get_stt_adapter
from helomi_core.tts import TTSWorker, get_tts_adapter
...
class Runtime(AbstractAsyncContextManager):
    ...
    async def get_parrot_extension(self, is_enabled=True) -> ParrotExtension:
        async def _creator() -> ParrotExtension:
            return ParrotExtension(
    ...
    async def get_server_extension(self, is_enabled=True) -> ServerExtension:
