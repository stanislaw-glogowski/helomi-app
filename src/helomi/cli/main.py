...
from helomi.common.events import EventBus, EventSubscription, ShutdownEvent
...
async def run() -> None:
    ...
    with EventBus() as event_bus, HuggingFaceProgress(progress):
        ...
        bridge_task = asyncio.create_task(
            bridge.run(event_bus),
            name="helomi-ui-events",
        )
        logger_task = asyncio.create_task(
            run_event_logger(event_bus), name="helomi-logs"
        )
