...
from helomi.resources import Profile, ProfileEntry
...
class ProfilePicker(ModalScreen[Profile | None]):
    ...
    def __init__(
        ...
        default_id = next(
            (
                ...
                if entry.id == default_profile_id and entry.is_valid
            ),
            next((entry.id for entry in profiles if entry.is_valid), None),
        )
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="profile-dialog"):
            yield Static("HELOMI  /  START", id="profile-kicker")
...
class StartupScreen(ModalScreen[bool | None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="startup-dialog"):
            yield Static("AGENT  /  INITIALIZING", id="startup-kicker")
