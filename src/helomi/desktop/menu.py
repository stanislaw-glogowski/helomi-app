...
class MenuBarApp:
    ...
    def _render(self) -> None:
        ...
        if snapshot.retry_enabled:
            ...
            items.append(retry)
        app_status = rumps.MenuItem(f"App: {snapshot.mode.presentation}")
        app_status.set_callback(None)
        agent_status = rumps.MenuItem(f"Agent: {snapshot.agent_activity.presentation}")
        agent_status.set_callback(None)
        items.extend((app_status, agent_status))
