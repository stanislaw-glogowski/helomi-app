...
def resolve_helper_executable() -> Path:
    configured = os.environ.get("HELOMI_AUDIO_HELPER")
    if configured:
        return _require_executable(Path(configured).expanduser())

    adapter_dir = Path(__file__).resolve().parent
    packaged = adapter_dir / "bin" / "audio"
    ...
    installed = shutil.which("audio")
    if installed is not None:
        return Path(installed)

    raise RuntimeError(
        "Native audio helper executable was not found; run "
        "native/macos/build.sh or set HELOMI_AUDIO_HELPER"
    )
