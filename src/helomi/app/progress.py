...
class ProgressStore:
    ...
    def __init__(self) -> None:
        ...
        self._listeners: set[Callable[[ProgressSnapshot], None]] = set()
        self._lock = Lock()
        self._sequence = 0
    ...
    def subscribe(
        self, listener: Callable[[ProgressSnapshot], None]
    ) -> Callable[[], None]:
        with self._lock:
    ...
    def begin(
        self, description: str, completed: float, total: float | None, unit: str
    ) -> int:
        with self._lock:
            ...
            self._items[item_id] = ProgressItem(
                item_id, description, completed, total, unit, ProgressStatus.ACTIVE
            )
        self._notify()
        return item_id
        ...
        *,
        ...
        with self._lock:
            ...
            self._items[item_id] = ProgressItem(
                ...
                completed if completed is not None else item.completed,
                total if replace_total else item.total,
                item.unit,
                item.status,
            )
        self._notify()
...
class _ProgressTqdm:
    ...
    def _update_store(
        self, *, description: str | None = None, replace_total: bool = False
    ) -> None:
        store = self.store
        if store is not None:
            store.update(
...
class HuggingFaceProgress(AbstractContextManager):
    ...
    def __enter__(self) -> Self:
        ...
        snapshot_module.hf_hub_download = self._with_progress(
            self._original_internal_hf_hub_download, force=True
        )
        return self
